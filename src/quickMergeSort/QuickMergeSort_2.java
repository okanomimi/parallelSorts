...
/**
 * this class is parallel sort.And this sort take quicksort and mergesort.
 * First, this sort divide arrays into min arrays.And it sort in each other.
 * then it takes Arrays.sort.
 * And each arrays that sorted merge each other by parallel merge.
 * @author okano
 *
 * @param <E> 	E must extends Comparable interface.
 */
public class QuickMergeSort_2<E extends Comparable> {
	private int[] posPointer;		//save next position
	private boolean[] isSorted;   //ソート完了部分と未完成部分の判定用   //ソート完了部分と未完成部分の判定用
	private final int SMALL = 20;	//if number of array lower than "SMALL" this sort takes Arrays.sort
	private int taskNum;
	...
	private void init(E[] array) {
		...
		int threadsNum;
		if(arrayLength <= 100000)		//if number of array's factor lower than 100000 , this sort just takes 5thread 
			threadsNum = 5;
		else
		...
		workers = new LinkedList<Callable<Object>>();		// this list is used to manage threads.

		isSorted = new boolean[arrayLength];				//judgment weather sorted or not sorted.
	...
	/**
	 * this method throw tasks to threads.
	 * @param array
	 * @param buff
