...
/**
 * UpDownSort is one of the parallel sort.
 * this sorter is //利用している。
 * @author okano
 * @version 10.3
 * @param <E>
 */
public class UpDownSort<E extends Comparable> {
				...
				while (array[right].compareTo(array[right + 1]) > 0) {// どこまで降順か調べる
					if (right + 1 >= arrayLength - 1) { // 最後まで降順であれば
						if (unitCount == 0) { // 二つのセットでなければ
							...
							settingTask(array, buff, left, right + 1,
									ALLREVERSE); // 仕事をセットする
							return;
						} else { // 二つのセットであれば
							right++; // 次に進める
	...
	// 二つセットで仕事がある場合
	private void settingTask(E[] array, E[] buff, int left, int mid, int right,
			int pattern) {
		updatePosPointer(left, right);
		futures.add(executor.submit(Executors.callable(new MergeSortWorker(
				array, buff, left, mid, right, pattern))));
	}

	private void settingTask(E[] array, E[] buff, int left, int right,
			int pattern) {
		updatePosPointer(left, right);
		futures.add(executor.submit(Executors.callable(new MergeSortWorker(
				array, buff, left, right, pattern))));
	}
	...
	class MergeSortWorker implements Runnable {
		...
		public MergeSortWorker(E[] array, E[] buff, int left, int mid,
				int right, int pattern) {
		...
		public void merge(E[] array, E[] buff, int left, int mid, int right) {
			...
			if (right - left < 7) {
				for (int i = left; i <= right; i++) {
					for (int j = i; j > left
							&& array[j - 1].compareTo(array[j]) > 0; j--)
						swap(array, j, j - 1);
				}
				return;
			}
			...
			while (i <= mid && j <= right) {
				...
				if (array[i].compareTo(array[j]) < 0) {
					...
					if (add >= 4) {
						i = binarySearch(array, lastPos, endPos, array[j],
								endPos);
						System.arraycopy(array, lastPos, buff, k, i - lastPos);
						k += i - lastPos;
					} else {
		...
		public void mergeUpDown(E[] array, E[] buff, int left, int mid,
				int right) {
		...
		public void mergeDownUp(E[] array, E[] buff, int left, int mid,
				int right) {
		...
		public void mergeDownDown(E[] array, E[] buff, int left, int mid,
				int right) {
	...
	/**
	 * check that left array is sorted. 
	 * @param left 
	 * @param right
	 * @return 
	 */
	private boolean checkLeftSideIsSorted(int left, int right) {
		...
		synchronized (isSorted) {
			...
			if (isSorted[left] && isSorted[left - 1]) { // if left side is sorted
